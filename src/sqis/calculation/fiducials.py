...
class getFiducials:
    ...
    def peaks_heartpy(self, signal_window):
        try:
            working_data, measures = hp.process(signal_window, sample_rate=self.fs, clean_rr=True)
    ...
    def peaks_elgendi(self, signal_window):
        ...
        """
        #Square the signal
        signal_sq = np.power(signal_window, 2)

        #Define moving average window sizes
        fast_window = int(0.11 * self.fs) | 1 #fast window for expected ~0.11s-wide systolic peak
        slow_window = int(0.60 * self.fs) | 1 #slow window for expected ~0.60s-long heartbeat
